...
namespace G3MagnetBoots
{
    ...
    ///
    /// Boom version with entry/exit transitions:
    /// - Uses a ThroughTheEyes-style parent boom only while EVA LOCKED + on hull.
    /// - Entry blends from stock camera pose to boom pose.
    /// - Exit blends from boom pose to stock camera pose.
    /// - Vessel switching / scene changes still hard-release and force FREE.
    /// </summary>
    ...
    internal static class Patch_FlightCamera_LateUpdate_EvaLockedBoom
    {
        ...
        private static bool Prefix(FlightCamera __instance)
        {
            ...
            // Leaving LOCKED/boom ownership: release before stock LateUpdate so stock computes its target pose.
            // The postfix then blends from the last boom pose to that stock pose.
    ...
    internal sealed class EvaLockedBoomCameraLifecycle : MonoBehaviour
    {
        private void Start()
        {
    ...
    internal static class EvaLockedBoomCamera
    {
        ...
        private const string BoomRootName = "G3 EVA Hull Locked Camera Boom";
    ...
    internal static class ThroughTheEyesCompat
    {
        private static bool _resolved;
        private static System.Type _firstPersonEVAType;
        private static FieldInfo _instanceField;
        private static FieldInfo _fpCameraManagerField;
        private static FieldInfo _isFirstPersonField;
        ...
        private static void Resolve()
        {
            ...
            _firstPersonEVAType = AccessTools.TypeByName("FirstPerson.FirstPersonEVA");
            if (_firstPersonEVAType == null)
                return;

            _instanceField = AccessTools.Field(_firstPersonEVAType, "instance");
            _fpCameraManagerField = AccessTools.Field(_firstPersonEVAType, "fpCameraManager");

            System.Type cameraManagerType = AccessTools.TypeByName("FirstPerson.FirstPersonCameraManager");
            if (cameraManagerType != null)
                _isFirstPersonField = AccessTools.Field(cameraManagerType, "isFirstPerson");
        }

        internal static bool IsFirstPersonActive()
        {
            try
            {
                Resolve();

                if (_instanceField == null || _fpCameraManagerField == null || _isFirstPersonField == null)
                    return false;
                ...
                object manager = _fpCameraManagerField.GetValue(instance);
                if (manager == null)
                    return false;

                object value = _isFirstPersonField.GetValue(manager);
